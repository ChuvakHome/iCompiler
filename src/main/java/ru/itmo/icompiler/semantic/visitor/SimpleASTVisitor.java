...
public class SimpleASTVisitor extends AbstractASTVisitor {
	...
	private void tryAddVariableToScope(VariableDeclarationASTNode node, SemanticContext ctx) {
	...
	private SemanticContext visitRoutineArgDecl(VariableDeclarationASTNode node, SemanticContext ctx) {
		if (ctx.getScope().lookupEntity(node.getVarName()) == null)
			tryAddVariableToScope(node, ctx);
	...
	public SemanticContext visit(VariableDeclarationASTNode node, SemanticContext ctx) {
		...
		if (varType == VarType.AUTO_TYPE) { /* check for type inference need */
			VariableAssignmentASTNode assignNode = (VariableAssignmentASTNode) node.getChild(0);
			...
			VariableAssignmentASTNode assignNode = node.getChildren().isEmpty() ? null : (VariableAssignmentASTNode) node.getChild(0);
	...
	public SemanticContext visit(ReturnStatementASTNode node, SemanticContext ctx) {
		try {
			node.getResultNode().checkType(ctx, currentRoutineType.getReturnType());
		} catch (CompilerException e) {
			ctx.addCompilerError(e);
		}
